...
For each tokenizer, computes per-sentence fertility gap (native_informal vs
romanized_informal) and tests H1: native-script fertility > romanized using a
Wilcoxon signed-rank test with bootstrap 95% CI on the median gap.

Writes:
  results/script_gap_significance.json  -- full stats per tokenizer
  results/per_sentence_gaps.csv         -- raw per-sentence fertility values

Requires matched corpora: romanized_informal must be line-aligned with
native_informal (guaranteed when generated via collect_data.py transliteration).
"""
...
def main() -> None:
    ...
    for name, count_fn in tokenizers.items():
        ...
        gap_df = per_sentence_gap(native_lines, romanized_lines, count_fn)
        ...
        try:
            ...
            print(
                ...
                f"native_costlier={stats['pct_pairs_native_costlier']}%"
            )
