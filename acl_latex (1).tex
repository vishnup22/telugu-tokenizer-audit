...
than English speakers for the same content. We audit six tokenizers
...
Telugu; dravidian-gpt2-telugu needs only 1.60, a 4.8$\times$ gap from
...
moderate fertility penalty, while dravidian-gpt2-telugu, the most
...
This paper is a focused empirical audit aimed at that gap. We
...
\end{itemize}
We find that tokenizers trained on Indic data not only reach lower
...
more to process than English. Our work narrows this line of inquiry
...
English subwords. These findings motivate our own morphological
...
\citep{verma-etal-2024-milu} used in our evaluation.
\citet{kunchukuttan-2020-indicnlp} provides the transliteration
tools behind our romanized corpus. Even with this growing ecosystem,
...
natural point of comparison. Our audit is the first to evaluate and
...
We build a three register Telugu corpus that supports both aggregate
fertility analysis and paired significance testing across script
varieties.

\paragraph{Native formal.}
We sample 789 sentences from the Telugu Wikipedia corpus
...
We sample 1,000 sentences from Telugu sentiment analysis and hate
...
We build a line by line romanized version of the native informal
...
exactly with its native script counterpart, which lets us run
...
We evaluate six tokenizers across three tiers of Indic language
...
dravidian-gpt2-telugu & SentencePiece                  & 32k  \\
\hline
\end{tabular}%
}
\caption{Tokenizers evaluated in this study with their backends
         and vocabulary sizes.}
\label{tab:tokenizers}
\end{table}

\paragraph{GPT-4o.}
We get token counts from the \texttt{tiktoken} library using the
\texttt{o200k\_base} encoding, the same tokenizer that GPT-4o uses.

\paragraph{Claude Haiku 4.5.}
We get token counts through the Anthropic
...
we use the same model through the Anthropic Messages API, accessed
in June 2026.

\paragraph{GPT-2 (legacy).}
We load the original GPT-2 tokenizer \citep{radford-etal-2019-gpt2}
...
and serves as our worst case baseline. It encodes Telugu Unicode
characters at the byte level, which drives its fertility extremely
high.

\paragraph{Sarvam-2B and Sarvam-105B.}
We load both Sarvam models \citep{sarvam-2024} via
\texttt{AutoTokenizer.from\_pretrained}, from
...
\paragraph{dravidian-gpt2-telugu.}
This model \citep{pulipakav-2026-dravidian} is built specifically
for Telugu and uses a 32k SentencePiece vocabulary trained only on
Telugu text. We load the tokenizer directly from the
\texttt{tokenizer.model} file via
\texttt{huggingface\_hub.hf\_hub\_download}, since
\texttt{AutoTokenizer} cannot read this model's custom vocabulary
format.

\subsection{Fertility and Script Fairness Metrics}

\paragraph{Fertility.}
Following \citet{rust-etal-2021-how}, we define the fertility of
...
We define the script fairness gap for tokenizer $t$ as the ratio of
...
significant, we run the Wilcoxon signed rank test
...
sentence pairs ($n = 1{,}000$). We also compute 95\% bootstrap
...
fertility hit, we build a minimal pair dataset of 62 Telugu word
...
\textit{verb\_agglutination\_chain}. We tokenize each word with all
six tokenizers and report the mean and median token count per
category. We release the dataset with our repository so others can
build on it.

\subsection{Benchmark Evaluation}
\label{sec:benchmark}

We evaluate downstream Telugu understanding on MILU
...
test split. We use three evaluation protocols, each matched to what
the model can actually do.

\paragraph{Zero shot generative (Claude Haiku 4.5).}
Each question is formatted as a four option MCQ prompt. We use the
Anthropic assistant prefill technique: we append
\texttt{\small\{"role": "assistant", "content": "The answer is"\}}
to the message list to push the model toward a single letter
response, with \texttt{max\_tokens=50} and greedy decoding. We parse
responses with a multi strategy letter extractor; 7,108 of the
7,304 responses (97.3\%) give us a parseable letter.

\paragraph{Log likelihood (dravidian-gpt2-telugu).}
Since dravidian-gpt2-telugu is a base language model without
instruction following ability, we evaluate it with the log
likelihood method: for each option $o_k$, we compute the length
...
We report GPT-4o's 5 shot accuracy from Table~3 of
\citet{verma-etal-2024-milu} (72.53\%). Running our own full
inference pass was too expensive, so we flag the methodological
...
We exclude Sarvam-2B from our accuracy analysis as a result, and
...
dravidian-gpt2-telugu & \textbf{1.68} & \textbf{1.60} &  5.49 \\
\bottomrule
\end{tabular}%
}
\caption{Fertility (tokens/word) by tokenizer and register.
         Lower is more efficient. Bold = best per register.}
\label{tab:fertility}
\end{table}

Fertility varies a lot across tokenizers. On the native informal
register, which is the closest proxy we have for everyday Telugu
usage, dravidian-gpt2-telugu reaches the lowest fertility at 1.60
...
higher than dravidian-gpt2-telugu.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig1_fertility_bar}
  \caption{Tokenizer fertility across three corpus registers.
           dravidian-gpt2-telugu achieves the lowest native-script
           fertility (1.60); GPT-2 the highest (22.08).}
  \label{fig:fertility-bar}
\end{figure}
...
five or more tokens to 97.8\% of words; at that rate it is
essentially falling back to byte level encoding for Telugu Unicode.
dravidian-gpt2-telugu, by contrast, keeps 69.7\% of words within one
or two tokens. That fits with segmentation close to the morpheme
level, in a language where words are typically just one to three
morphemes long.

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig4_token_length_dist}
  \caption{Token-length distribution on the native informal register.
           dravidian-gpt2-telugu encodes 61.6\% of words as a single
           token; Claude Haiku~4.5 and GPT-2 assign 5+ tokens to
           over 82\% of words.}
...
dravidian-gpt2-telugu & \textbf{0.29} & $-$3.88 & [$-$4.00,\;$-$3.83] & 1.0 \\
\bottomrule
\end{tabular}%
}
\caption{Script fairness gap $\mathcal{G}(t)$, median per-sentence
         gap $\delta$ (tokens/word), 95\% bootstrap CI, and
         Wilcoxon signed-rank $p$-value ($n=1{,}000$ pairs).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig2_gap_violin}
  \caption{Per-sentence fertility gap distribution ($n=1{,}000$
           matched pairs). Orange violins (above zero) indicate
           native script is costlier; green violins (below zero)
           indicate romanized is costlier.}
  \label{fig:violin}
\end{figure}
...
($\mathcal{G} = 0.68$), and dravidian-gpt2-telugu ($\mathcal{G} =
0.29$) all make native script cheaper than romanized input, with
native script costlier in only 0.0\% to 0.2\% of pairs.
dravidian-gpt2-telugu shows the strongest effect: native Telugu
script costs 3.4$\times$ less than its romanized equivalent, the
largest native script advantage we observed. GPT-4o sits in the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig3_heatmap_morph}
  \caption{Mean tokens per word by morphological category
           (log-scale color). Honorific suffixes and verb
           agglutination chains incur the highest penalties
           under general-purpose tokenizers.}
  \label{fig:heatmap}
\end{figure}
...
22 languages instead of just one. dravidian-gpt2-telugu does even
...
\textit{base\_noun} form in our set as a single token (mean = 1.0).
That points to a 32k vocabulary large enough to store common Telugu
...
Table~\ref{tab:milu} reports MILU Telugu test accuracy for every
model we have a score for.
...
dravidian-gpt2-telugu & Log likelihood (ours) & 1.60 & 26.97\% \\
\hline
Chance baseline       & N/A                 & N/A  & 25.00\% \\
\hline
\end{tabular}%
}
\caption{MILU Telugu test accuracy alongside native informal
         fertility. Fertility and accuracy are not monotonically
         related.}
\label{tab:milu}
\end{table}
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig5_fertility_vs_accuracy}
  \caption{Fertility vs.\ MILU Telugu accuracy. The absence of a
           negative correlation confirms that tokenizer efficiency
           and downstream capability are orthogonal.}
  \label{fig:scatter}
\end{figure}
These numbers show that tokenizer fertility and downstream accuracy
are decoupled. GPT-4o gets the highest accuracy, 72.53\%, with only
a moderate fertility of 3.25. Claude Haiku~4.5 scores 60.78\%
despite having the worst fertility among the instruction tuned
models we tested (7.73). dravidian-gpt2-telugu has the best
...
tokenizer. We read the token tax as a pricing problem sitting at the
...
word. The 200,000 token context window works out to about 25,873
...
Our results reframe tokenizer fertility as a question of economic
...
more efficiently. dravidian-gpt2-telugu pushes this furthest, with a
...
worst fertility of any instruction tuned model we tested (7.73).
dravidian-gpt2-telugu has the best fertility of all, 1.60, and still
...
so we excluded it. The tokenizer is well calibrated; the model just
...
Our romanized corpus relies only on ITRANS transliteration. Other
...
numbers. We treat ITRANS as a reasonable worst case for general
purpose tokenizers, but the exact size of the gap will shift with
romanization style.

\paragraph{Corpus size and domain coverage.}
Our corpus of 2,789 sentences is large enough for significance
...
We only study Telugu here. The script fairness gap will probably
...
Excluding Sarvam-2B from the accuracy analysis, 92.1\% of its
responses were unparseable, limits what we can say about whether
Indic optimised tokenizers paired with instruction tuned models
close the capability gap. Future work should score Sarvam models
...
tokenizers and three corpus registers. Our findings answer all
four research questions in turn.

Fertility varies by up to 13.8$\times$ across tokenizers on the same
native Telugu text: dravidian-gpt2-telugu sits at one extreme (1.60
...
4.66$\times$. Tokenizers trained on Indic data invert that bias:
native script ends up cheaper than romanized input ($\mathcal{G}$
between 0.29 and 0.68, all $p < 10^{-165}$ or $p = 1.0$,
$n = 1{,}000$) (RQ2). The penalty also follows morphological
...
numbers can diverge substantially, as our results show.
...
that do not depend on model scale at all. We release our corpus,
