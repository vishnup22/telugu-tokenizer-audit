...
LLM development, underrepresented in pretraining data and largely
absent from tokenizer design. This creates a \textit{token tax}:
native-script users pay more per API call and get a smaller effective
context window for the same content. I audit nine tokenizers across
four registers, native formal, native informal, a controlled
ITRANS-romanized informal corpus, and an English Wikipedia baseline,
totaling 4,038 sentences, plus 62 minimal pairs across nine
agglutinative morphological categories checked by one native speaker.
On native informal Telugu, fertility ranges from 1.60 tokens per word
for Anon-Indic-GPT2 to 22.08 for legacy GPT-2, a 13.8$\times$ gap.
At Claude Haiku~4.5, native informal Telugu measures 7.73 tokens per
word versus 1.46 on the directly measured English baseline, a
5.31$\times$ cost multiplier. On the controlled romanized corpus,
general-purpose tokenizers penalize native Telugu script, while
Indic-trained tokenizers reverse that penalty. I also source a 249-line organic Tenglish sample from public YouTube comments and use it as a separate robustness check. On MILU
\citep{verma-etal-2024-milu} (7,304 Telugu MCQ questions), tokenizer
efficiency and downstream accuracy are consistent with being
independently determined, though evaluation protocols differ across
models. The results place the token tax at the tokenizer and pricing
layer rather than the model-capability layer.
\end{abstract}
...
Telugu makes this problem concrete. Spoken by over 96 million people according to the most recent
available census data \citep{census2011}, Telugu is a
...
evaluate nine tokenizers, general purpose commercial APIs, open
source baselines built for English, multilingual tokenizers, and
tokenizers built specifically for Indic languages, across four corpus
registers and a morphologically annotated minimal pair dataset. Four
...
\begin{itemize}
    ...
    \item \textbf{RQ4} (Independence): Does tokenizer efficiency
    co-vary with downstream task accuracy in this study, or do
    efficiency and accuracy reflect different design dimensions?
\end{itemize}
My contributions are:
\begin{itemize}
    \item A fertility audit of nine tokenizers across 4,038 sentences
    in four registers, including a direct English baseline, per-sentence
    Wilcoxon signed-rank tests, rank-biserial effect sizes, and bootstrap
    confidence intervals on script fairness gaps (\S\ref{sec:results-fertility}, \S\ref{sec:results-gap});
    \item A morphological breakdown built from 62 minimal pairs
    checked by one native speaker of Coastal Andhra Telugu, spanning
    nine agglutinative
    ...
    downstream accuracy need not co-vary, with evaluation protocol
    caveats (\S\ref{sec:results-milu});
...
paired significance tests sentence by sentence. Because the
romanized corpus is ITRANS-generated rather than organic Tenglish, I
treat it as a controlled worst-case baseline for romanized Telugu
input rather than as a direct sample of everyday online usage.

\paragraph{English baseline.}
I additionally sample 1,000 sentences from English Wikipedia using
the same sentence-length filter used for the Telugu corpora. This
register is not paired with the Telugu data, but it provides a direct
English fertility baseline for each tokenizer.

\paragraph{Preprocessing.}
Every register goes through the same cleaning pipeline: regex based
removal of personally identifiable information, deduplication on
normalized surface form, and length filtering. The final corpus has
4,038 sentences in total: 789 native formal, 1,000 native informal,
1,000 romanized informal, 1,000 English Wikipedia sentences, and 249 organic Tenglish comments.

\subsection{Tokenizers}

I evaluate nine tokenizers across four tiers of Telugu coverage;
Table~\ref{tab:tokenizers} summarizes them.
\begin{table}[H]
\centering
...
XLM-R                 & HuggingFace SentencePiece           & 250k \\
IndicBERT             & HuggingFace WordPiece               & 200k \\
Llama~3               & HuggingFace BPE                     & 128k \\
...
\paragraph{XLM-R, IndicBERT, and Llama 3.}
I additionally load the tokenizer backends for XLM-R, IndicBERT, and
Llama~3 through \texttt{transformers.AutoTokenizer}. These add one
multilingual SentencePiece baseline, one Indic-pretrained encoder
baseline, and one modern frontier tokenizer to the comparison set.
\paragraph{GPT-4o.}
...
\texttt{sarvamai/sarvam-2b-v0.5} and \texttt{sarvamai/sarvam-105b}
...
Here $\mathrm{words}(s)$ is the word count of sentence $s$. I compute
this both by white-space splitting and with IndicNLP tokenization as
a sensitivity check; the main text reports the white-space version
for comparability, with the alternative counts released alongside the
results. Lower fertility means more efficient tokenization.
...
significant, I run a two-sided Wilcoxon signed rank test
\citep{wilcoxon-1945} on the per sentence fertility gaps
$\delta_i = \mathcal{F}(t, s_i^{\mathrm{nat}})
- \mathcal{F}(t, s_i^{\mathrm{rom}})$ across the 1,000 matched
sentence pairs ($n = 1{,}000$). I report direction separately from
the sign of the median gap and include rank-biserial correlation as
the primary effect size. I also compute 95\% BCa bootstrap
confidence intervals on the median gap using 10,000 resamples.
...
forms checked by one native speaker of Coastal Andhra Telugu,
spanning nine agglutinative categories: \textit{base\_noun}, \textit{case\_suffix},
...
\textit{verb\_agglutination\_chain}. I tokenize each word with the historical core six tokenizers and report the mean and median token count per
...
each tokenizer across the four main corpus registers and a Tenglish robustness sample.
\begin{table}[H]
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Tokenizer} & \textbf{Native Formal} & \textbf{Native Informal} & \textbf{Romanized Informal} & \textbf{English} \\
\midrule
GPT-4o                &  3.28 &  3.25 &  3.98 & 1.33 \\
Claude Haiku~4.5      &  6.91 &  7.73 &  4.95 & 1.46 \\
GPT-2 (legacy)        & 19.99 & 22.08 &  4.74 & 1.34 \\
...
Anon-Indic-GPT2       & \textbf{1.68} & \textbf{1.60} &  5.49 & 2.65 \\
\bottomrule
\end{tabular}%
}
\caption{Fertility (tokens/word) by tokenizer and register. Bold = best on the Telugu registers.}
\label{tab:fertility}
\end{table}
Fertility varies sharply across tokenizers. On the native informal
...
text. Claude Haiku~4.5 produces 7.73 tokens per word, while Llama~3
reaches 14.15, much closer to the legacy English-centric baselines
than to the Indic-specialized tokenizers.

The Sarvam models, XLM-R, and IndicBERT all reach native informal
fertility between 2.41 and 3.01, close to GPT-4o's 3.25. A direct
English baseline also shows that Claude's native informal Telugu
fertility is 5.31$\times$ its English fertility (7.73 versus 1.46),
while GPT-4o's multiplier is 2.44$\times$ (3.25 versus 1.33). This
suggests that deliberate multilingual tokenizer design can close
most of the fertility gap without giving up cross-lingual coverage.
...
together with the median per-sentence gap, BCa bootstrap confidence
intervals, rank-biserial effect size, and two-sided Wilcoxon result.
\begin{table}[H]
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lrrrrr@{}}
\toprule
\textbf{Tokenizer} & $\mathcal{G}(t)$ & \textbf{Med.\ $\delta$} & \textbf{95\% CI} & \textbf{$r_{rb}$} & \textbf{$p$} \\
\midrule
GPT-4o                & 0.82 & $-$0.73 & [$-$0.76,\;$-$0.68] & $-$0.91 & $<10^{-132}$ \\
Claude Haiku~4.5      & 1.56 & $+$2.75 & [$+$2.71,\;$+$2.81] & 1.00 & $<10^{-164}$ \\
GPT-2 (legacy)        & 4.66 & $+$17.22 & [$+$17.00,\;$+$17.42] & 1.00 & $<10^{-164}$ \\
XLM-R                 & 0.61 & $-$1.55 & [$-$1.60,\;$-$1.50] & $-$1.00 & $<10^{-162}$ \\
IndicBERT             & 0.89 & $-$0.40 & [$-$0.40,\;$-$0.36] & $-$0.93 & $<10^{-129}$ \\
Llama~3               & 3.29 & $+$9.79 & [$+$9.64,\;$+$9.92] & 1.00 & $<10^{-164}$ \\
Sarvam-2B             & 0.53 & $-$2.65 & [$-$2.70,\;$-$2.58] & $-$1.00 & $<10^{-164}$ \\
Sarvam-105B           & 0.68 & $-$1.30 & [$-$1.33,\;$-$1.25] & $-$1.00 & $<10^{-162}$ \\
Anon-Indic-GPT2       & 0.29 & $-$3.88 & [$-$4.00,\;$-$3.83] & $-$1.00 & $<10^{-164}$ \\
\bottomrule
\end{tabular}%
}
\caption{Script fairness gap $\mathcal{G}(t)$, median $\delta$, 95\% CI,
         rank-biserial effect size, and two-sided Wilcoxon $p$ ($n=1{,}000$).
         $\mathcal{G}(t){>}1$: native costlier; $\mathcal{G}(t){<}1$: romanized costlier.}
\label{tab:gap}
\end{table}
The results still split cleanly along tokenizer origin. Western,
general-purpose tokenizers penalize native Telugu script most
strongly: GPT-2 imposes a 4.66$\times$ penalty, Llama~3 a 3.29$\times$
penalty, and Claude Haiku~4.5 a 1.56$\times$ penalty. In all three
cases the median gap and rank-biserial effect size point in the same
direction.

Tokenizers trained on Indic data flip this bias the other way.
Sarvam-2B ($\mathcal{G}=0.53$), Sarvam-105B ($\mathcal{G}=0.68$),
IndicBERT ($\mathcal{G}=0.89$), and Anon-Indic-GPT2
($\mathcal{G}=0.29$) all make native script cheaper than the
ITRANS-transliterated baseline, while XLM-R also favors native script
($\mathcal{G}=0.61$). GPT-4o sits near neutral at
$\mathcal{G}=0.82$, with a much smaller effect than the English-centric
baselines.
...
morphological category, across the historical core six tokenizers.
\begin{table}[H]
\centering
\resizebox{\columnwidth}{!}{%
...
\caption{Mean tokens/word by morphological type.
         ...
         S-2B=Sarvam-2B, S-105B=Sarvam-105B,
         Anon.=Anon-Indic-GPT2 \citep{anonymous-2026-indic}.
         Bold = lowest per row.}
\label{tab:minimal}
\end{table}
...
\subsection{RQ4: Tokenizer Efficiency and Downstream Accuracy}
...
Claude Haiku~4.5      & 0-shot (ours)       & 7.73 & 60.92\% \\
...
\caption{MILU Telugu accuracy and native informal fertility.
         Sarvam-2B excluded: 7.9\% parse rate (576/7{,}304) makes
         generative accuracy uninterpretable; its paper-reported
         5-shot score of 28.57\% uses an incomparable setup.
         Evaluation protocols differ across models (see \S\ref{sec:benchmark}).}
\label{tab:milu}
\end{table}
These numbers are consistent with tokenizer efficiency and
downstream accuracy being independently determined, though this
conclusion should be read cautiously: the three models use
incompatible evaluation protocols (5-shot paper-reported vs.\ zero-shot
generative vs.\ log-likelihood), which prevents direct numerical
comparison. Within those constraints, the pattern still shows that
high capability does not require the most efficient tokenizer, and
the most efficient tokenizer does not imply strong downstream task
performance. GPT-4o gets the highest accuracy, 72.53\%, with a
moderate fertility of 3.25. Claude Haiku~4.5 scores 60.92\% despite
having the worst fertility among the instruction-tuned models I
tested (7.73). Anon-Indic-GPT2 has the best fertility in the study
(1.60) and still scores near chance, 26.97\% against a 25.0\%
baseline. That is not a tokenizer problem: it is a 110M-parameter
base language model with no instruction tuning, too small for the
task regardless of how cheaply it tokenizes.
...
tokenizer. I therefore read the token tax as a pricing problem
sitting at the tokenizer level, separate from whatever the
underlying model can do, and something that has to be fixed there
rather than waiting on bigger models.
...
1,000 Telugu words costs about \$0.0062 in input charges, while the
directly measured English baseline of 1.46 tokens per word implies
only about \$0.0012 for the same word count. That is a measured
5.31$\times$ multiplier on Claude for native informal Telugu versus
English. GPT-4o's multiplier is smaller but still substantial at
2.44$\times$ (3.25 versus 1.33). None of this is incidental; it adds
another layer of disadvantage on top of everything else that comes
with speaking a low-resource language with little representation in
model training data.
...
fertility of 7.73 tokens per word on native Telugu means 1,000 Telugu
words cost about \$0.0062 in input charges alone, while the directly
measured English baseline for the same tokenizer costs about \$0.0012.
At GPT-4o, the comparable multiplier is 2.44$\times$ rather than
Claude's 5.31$\times$. These costs stack across every interaction,
and the same token inflation also shrinks the amount of Telugu text
that fits into a fixed context window. For someone paying for AI
access out of a prepaid plan, which is how many people in India get
online, this is not an abstract number. It is a hard ceiling on how
much of the model they can actually use per rupee.
...
\subsection{Tokenizer Efficiency and Capability Context}

The MILU results in Table~\ref{tab:milu} are consistent with
tokenizer efficiency and downstream accuracy not moving together,
with the caveat that evaluation protocols differ across models
(see \S\ref{sec:benchmark}). GPT-4o
gets the highest accuracy, 72.53\%, at a moderate fertility of 3.25.
Claude Haiku~4.5 comes in second at 60.92\%, despite having the
...
This independence matters for how the field should prioritize its
...
numbers. I therefore treat ITRANS as a controlled worst-case
baseline for romanized Telugu rather than as a direct sample of
organic Tenglish, and the exact size of the gap will shift with
romanization style.

\paragraph{Corpus size and domain coverage.}
My corpus of 4,038 sentences is large enough for significance
...
and checked by one native speaker of Coastal Andhra Telugu. A larger
validation effort across Rayalaseema and Telangana speakers, with
formal inter-annotator agreement, would make the morphological
analysis stronger.
...
fertility and script fairness for Telugu, covering nine LLM
tokenizers and four main corpus registers and a Tenglish robustness sample. My findings answer all
...
tokenizers impose a native-script penalty between 1.56$\times$ and
4.66$\times$, while tokenizers trained on Indic data invert that
bias: native script ends up cheaper than romanized input
($\mathcal{G}$ between 0.29 and 0.68) (RQ2). The penalty also
follows morphological
...
that define Telugu as an agglutinative language (RQ3). Tokenizer fertility and downstream MILU accuracy are consistent
with being independently determined in this study: the model with
the best tokenizer scores near chance, 26.97\%, while the model
with the worst tokenizer among instruction-tuned models scores
60.92\%. That gap is the clearest evidence in the paper that the
token tax operates at the pricing layer rather than the
model-capability layer (RQ4).
...
reported alongside fertility for multilingual tokenizer evaluation cards
...
access AI services on limited budgets, and the directly measured
English comparison in this study shows that the same provider can
charge several times more tokens for Telugu than for English.
Closing that gap takes deliberate engineering choices at the
tokenizer level, choices that do not depend on model scale at all.
I release my corpus, minimal pair dataset, evaluation scripts, and
every other experimental artifact, so other researchers can build on
this work for Telugu and for other morphologically rich, low-resource
languages.
