...
This model \citep{anonymous-2026-indic} is built specifically
for Telugu and uses a 32k SentencePiece vocabulary trained only on
Telugu text. We load the tokenizer directly from its custom SentencePiece
vocabulary file, since standard auto-classes cannot read it.
...
\caption{Mean tokens/word by morphological type.
         ...
         S-2B=Sarvam-2B, S-105B=Sarvam-105B, Drav.=Anon-Indic-GPT2.
         Bold = lowest per row.}
\label{tab:minimal}
\end{table}
...
models we tested (7.73). Anon-Indic-GPT2 has the best
...
what we can say about whether Indic optimised tokenizers paired with
