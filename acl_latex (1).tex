...
and accuracy do not co-vary: GPT-4o scores 72.53\% (5-shot, from
prior work) despite a moderate fertility penalty, while
Anon-Indic-GPT2, the most efficient tokenizer, scores 26.97\%,
near chance. The token tax reflects pricing, not capability.
\end{abstract}
...
accuracy being decoupled, though this conclusion should be read
cautiously: the three models use incompatible evaluation protocols
(5-shot paper-reported vs.\ zero-shot generative vs.\ log-likelihood),
which prevents direct comparison. Within those constraints, the
pattern holds across all three models in the same direction. GPT-4o gets the highest accuracy, 72.53\%, with only
...
that define Telugu as an agglutinative language (RQ3). Tokenizer fertility and downstream MILU accuracy do not co-vary
in this study: the model with the best tokenizer scores near
chance, 26.97\%, while the model with the worst tokenizer among
instruction-tuned models scores 60.78\%. That gap is the clearest evidence in the paper that
